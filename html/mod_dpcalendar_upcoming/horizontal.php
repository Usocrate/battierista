...
?>
<div class="container-fluid mod-dpcalendar-upcoming mod-dpcalendar-upcoming-horizontal mod-dpcalendar-upcoming-<?php echo $module->id; ?> dp-locations"
	...
	<div class="mod-dpcalendar-upcoming-horizontal__events">
		<?php foreach ($groupedEvents as $groupHeading => $events) { ?>
			<?php if ($groupHeading) { ?>
				<div class="mod-dpcalendar-upcoming-horizontal__group">
				<p class="mod-dpcalendar-upcoming-horizontal__heading dp-group-heading"><?php echo $groupHeading; ?></p>
			<?php } ?>
			<?php foreach ($events as $event) { ?>
				<?php $displayData['event'] = $event; ?>
				<?php $startDate = $dateHelper->getDate($event->start_date, $event->all_day); ?>
				<div class="mod-dpcalendar-upcoming-horizontal__event  dp-event dp-event_<?php echo $event->ongoing_start_date ? ($event->ongoing_end_date ? 'started' : 'finished') : 'future'; ?><?php echo $groupHeading ? '' : ' mod-dpcalendar-upcoming-horizontal__group'; ?>">
					<?php // echo $layoutHelper->renderLayout('block.flatcalendar', ['date' => $startDate, 'color' => $event->color]); ?>
					<div class="mod-dpcalendar-upcoming-horizontal__information" style="border-color:#<?php echo $event->color ?>">
						<?php if ($event->state == 3) { ?>
							<span class="dp-event_canceled">[<?php echo $translator->translate('MOD_DPCALENDAR_UPCOMING_CANCELED'); ?>]</span>
						<?php } ?>
						<h3 class="card-title"><a href="<?php echo $event->realUrl; ?>" class="dp-event-url dp-link"><?php echo $event->title; ?></a></h3>
	...
	<?php if ($params->get('show_map')) { ?>
		<div class="mod-dpcalendar-upcoming-horizontal__map dp-map"
